...
# ----------------------------
# Core lexicon (Palestinian Arabic)
# ----------------------------

_AR_DIGITS = {
    0: "صفر", 1: "واحد", 2: "تنين", 3: "تلاتة", 4: "أربعة", 5: "خمسة",
    6: "ستة", 7: "سبعة", 8: "تمانية", 9: "تسعة", 10: "عشرة",
    11: "حداشر", 12: "اطناشر", 13: "تلاتعشر", 14: "أربعتاشر", 15: "خمستاشر",
...
# Currency words: Israeli Shekel (ILS/NIS), Jordanian Dinar (JOD), or USD
CURRENCY = {
    "ILS": ("شيكل", "شيكلين", "شواكل", "أغورة", "أغورتين", "أغورات"),
    "NIS": ("شيكل", "شيكلين", "شواكل", "أغورة", "أغورتين", "أغورات"),
    "شيكل": ("شيكل", "شيكلين", "شواكل", "أغورة", "أغورتين", "أغورات"),
    "JOD": ("دينار", "دينارين", "دنانير", "قرش", "قرشين", "قروش"),
    "دينار": ("دينار", "دينارين", "دنانير", "قرش", "قرشين", "قروش"),
...
_AR_ORDINALS = {
    ...
    11: "الحداشر", 12: "الاطناشر"
}

_AR_MONTHS = {
    1: "كانون ثاني", 2: "شباط", 3: "آذار", 4: "نيسان", 5: "أيار", 6: "حزيران",
    7: "تموز", 8: "آب", 9: "أيلول", 10: "تشرين أول", 11: "تشرين تاني", 12: "كانون أول"
}
...
# ----------------------------
# Number to Palestinian Arabic
# ----------------------------

def int_to_palestinian_words(n: int) -> str:
    if n < 0:
        return "سالب " + int_to_palestinian_words(-n)
    if n < 20:
        return _AR_DIGITS[n]
    if n < 100:
        tens = (n // 10) * 10
    ...
    if n < 1000:
        ...
        return f"{_AR_HUNDREDS[hundreds]} و{int_to_palestinian_words(rest)}"
    ...
    for scale_value, scale_singular, scale_dual, scale_plural in _SCALES:
        if n >= scale_value:
            ...
            if major == 1:
                ...
                major_words = f"{int_to_palestinian_words(major)} {scale_plural}"
            else:
                major_words = f"{int_to_palestinian_words(major)} {scale_singular}"
            
            if rest:
                major_words += f" و{int_to_palestinian_words(rest)}"
            return major_words
...
def normalize_currency(text: str) -> str:
    def sym_first(m):
        ...
        if "." in t:
            ...
            if b_i == 0:
                return f"{int_to_palestinian_words(a_i)} {major_word}"
            
            minor_word = get_plural_form(b_i, minor_sg, minor_du, minor_pl)
            return f"{int_to_palestinian_words(a_i)} {major_word} و{int_to_palestinian_words(b_i)} {minor_word}"
    ...
    return re.sub(r"(?P<num>-?[\d٠-٩]+(?:[.,٫][\d٠-٩]+)?)\s*(?P<cur>ILS|NIS|JOD|شيكل|دينار)", num_first, text)
...
def normalize_dates(text: str) -> str:
    def repl_slash(m):
        ...
        day_word = _AR_ORDINALS.get(day, int_to_palestinian_words(day))
        month_word = _AR_MONTHS.get(month, int_to_palestinian_words(month))
        
        if year:
            year_word = int_to_palestinian_words(year)
            return f"{day_word} من {month_word} {year_word}"
        return f"{day_word} من {month_word}"
...
def normalize_time(text: str) -> str:
    def repl(m):
        ...
        h_words = int_to_palestinian_words(hh)
        
        if mm == 0: return f"الساعة {h_words}"
        if mm == 15: return f"{h_words} وربع"
        if mm == 30: return f"{h_words} ونص"
        if mm == 45:
            next_h = (hh + 1) % 24
            return f"{int_to_palestinian_words(next_h)} إلا ربع"
        
        m_words = int_to_palestinian_words(mm)
...
def normalize_text_for_tts_palestinian(text: str) -> str:
