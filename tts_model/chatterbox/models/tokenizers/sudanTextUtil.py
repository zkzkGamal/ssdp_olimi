...
# Currency words with proper pluralization (Sudanese Pound)
CURRENCY = {
    "SDG": ("جنيه", "جنيهين", "جنيهات", "قرش", "قرشين", "قروش"),
    "ج.س": ("جنيه", "جنيهين", "جنيهات", "قرش", "قرشين", "قروش"),
    "جنيه": ("جنيه", "جنيهين", "جنيهات", "قرش", "قرشين", "قروش"),
...
# Month names (Sudan uses Gregorian names like Egypt)
_AR_MONTHS = {
...
# ----------------------------
# Number to Sudanese Arabic
# ----------------------------

def int_to_sudanese_words(n: int) -> str:
    """Convert integer (>=0) into Sudanese Arabic words with better scaling."""
    if n < 0:
        return "سالب " + int_to_sudanese_words(-n)
    if n < 20:
        return _AR_DIGITS[n]
    if n < 100:
        tens = (n // 10) * 10
    ...
    if n < 1000:
        ...
        return f"{_AR_HUNDREDS[hundreds]} و{int_to_sudanese_words(rest)}"
    ...
    for scale_value, scale_singular, scale_dual, scale_plural in _SCALES:
        if n >= scale_value:
            ...
            if major == 1:
                ...
                major_words = f"{int_to_sudanese_words(major)} {scale_plural}"
            else:
                major_words = f"{int_to_sudanese_words(major)} {scale_singular}"
            
            if rest:
                major_words += f" و{int_to_sudanese_words(rest)}"
            return major_words
...
def num_token_to_words(token: str) -> str:
    ...
    if "." in t:
        a, b = t.split(".", 1)
        a_words = int_to_sudanese_words(int(a)) if a else "صفر"
        ...
        out = int_to_sudanese_words(int(t))
...
def normalize_currency(text: str) -> str:
    ...
    Handles: "100 SDG", "100ج.س", "$12.50", "€30"
    """
    def sym_first(m):
        sym = m.group("sym")
        num = m.group("num")
        ...
        if "." in t:
            ...
            if b_i == 0:
                return f"{int_to_sudanese_words(a_i)} {major_word}"
            
            minor_word = get_plural_form(b_i, minor_sg, minor_du, minor_pl)
            return f"{int_to_sudanese_words(a_i)} {major_word} و{int_to_sudanese_words(b_i)} {minor_word}"
    ...
    return re.sub(r"(?P<num>-?[\d٠-٩]+(?:[.,٫][\d٠-٩]+)?)\s*(?P<cur>SDG|USD|EUR|ج\.س|جنيه)", num_first, text)
...
def normalize_dates(text: str) -> str:
    ...
    def repl_slash(m):
        ...
        day_word = _AR_ORDINALS.get(day, int_to_sudanese_words(day))
        month_word = _AR_MONTHS.get(month, int_to_sudanese_words(month))
        
        if year:
            year_word = int_to_sudanese_words(year)
...
def normalize_time(text: str) -> str:
    ...
    def repl(m):
        ...
        if mm == 45:
            next_h = (hh + 1) % 24
            return f"{int_to_sudanese_words(next_h)} إلا ربع"
        
        m_words = int_to_sudanese_words(mm)
        if mm == 1:
...
def normalize_ranges(text: str) -> str:
    """Handle numeric ranges using the distinctive Sudanese 'لحدي' (li-haddi)."""
    def repl(m):
        a, b = m.group("a"), m.group("b")
        return f"من {num_token_to_words(a)} لحدي {num_token_to_words(b)}"
...
def normalize_phone_like(text: str) -> str:
    """Enhanced phone number handling (Sudanese patterns commonly start with 09 or 01)."""
    def repl(m):
...
def normalize_text_for_tts_sudanese(text: str) -> str:
    """
