...
_AR_DIGITS = {
    ...
    11: "ادعش", 12: "طنعش", 13: "تلطعش", 14: "أربعطعش", 15: "خمسطعش",
...
# Currency words with proper pluralization (Syrian Lira/Pound)
CURRENCY = {
    "SYP": ("ليرة", "ليرتين", "ليرات", "قرش", "قرشين", "قروش"),
    "ل.س": ("ليرة", "ليرتين", "ليرات", "قرش", "قرشين", "قروش"),
...
# Levantine Month names (Standard in Syria)
_AR_MONTHS = {
...
# ----------------------------
# Number to Syrian Arabic
# ----------------------------

def int_to_syrian_words(n: int) -> str:
    """Convert integer (>=0) into Syrian Arabic words with better scaling."""
    if n < 0:
        return "ناقص " + int_to_syrian_words(-n)
    if n < 20:
        return _AR_DIGITS[n]
    if n < 100:
        tens = (n // 10) * 10
    ...
    if n < 1000:
        ...
        return f"{_AR_HUNDREDS[hundreds]} و{int_to_syrian_words(rest)}"
    ...
    for scale_value, scale_singular, scale_dual, scale_plural in _SCALES:
        if n >= scale_value:
            ...
            if major == 1:
                ...
                major_words = f"{int_to_syrian_words(major)} {scale_plural}"
            else:
                major_words = f"{int_to_syrian_words(major)} {scale_singular}"
            
            if rest:
                major_words += f" و{int_to_syrian_words(rest)}"
            return major_words
...
def num_token_to_words(token: str) -> str:
    ...
    if "." in t:
        a, b = t.split(".", 1)
        a_words = int_to_syrian_words(int(a)) if a else "صفر"
...
def normalize_currency(text: str) -> str:
    ...
    Handles: "100 SYP", "100ل.س", "$12.50", "€30"
    """
    def sym_first(m):
        sym = m.group("sym")
        num = m.group("num")
        ...
        if "." in t:
            ...
            if b_i == 0:
                return f"{int_to_syrian_words(a_i)} {major_word}"
            
            minor_word = get_plural_form(b_i, minor_sg, minor_du, minor_pl)
            return f"{int_to_syrian_words(a_i)} {major_word} و{int_to_syrian_words(b_i)} {minor_word}"
    ...
    return re.sub(r"(?P<num>-?[\d٠-٩]+(?:[.,٫][\d٠-٩]+)?)\s*(?P<cur>SYP|USD|EUR|ل\.س|ليرة)", num_first, text)
...
def normalize_dates(text: str) -> str:
    ...
    def repl_slash(m):
        ...
        day_word = _AR_ORDINALS.get(day, int_to_syrian_words(day))
        month_word = _AR_MONTHS.get(month, int_to_syrian_words(month))
        
        if year:
            year_word = int_to_syrian_words(year)
...
def normalize_time(text: str) -> str:
    """Enhanced time handling with Syrian expressions."""
    def repl(m):
        hh = int(normalize_arabic_digits(m.group("hh")))
        mm = int(normalize_arabic_digits(m.group("mm")))
        
        h_words = int_to_syrian_words(hh)
        
        if mm == 0:
            return f"{h_words} تماما"
        if mm == 15:
        ...
        if mm == 45:
            next_h = (hh + 1) % 24
            return f"{int_to_syrian_words(next_h)} إلا ربع"
        
        m_words = int_to_syrian_words(mm)
        if mm == 1:
...
def normalize_text_for_tts_syrian(text: str) -> str:
    """
