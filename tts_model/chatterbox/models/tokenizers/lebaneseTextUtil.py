...
# ----------------------------
# Core lexicon (Lebanese)
# ----------------------------

_AR_DIGITS = {
    0: "صفر", 1: "واحد", 2: "تنين", 3: "تلاتة", 4: "أربعة", 5: "خمسة",
    6: "ستة", 7: "سبعة", 8: "تمانية", 9: "تسعة", 10: "عشرة",
    11: "حدعش", 12: "طنعش", 13: "تلطعش", 14: "أربعطعش", 15: "خمسطعش",
    16: "سطعش", 17: "سبعطعش", 18: "تمنطعش", 19: "تسعطعش"
}
...
_AR_HUNDREDS = {
    100: "مية", 200: "ميتين", 300: "تلاتمية", 400: "أربعمية",
    500: "خمسمية", 600: "ستمية", 700: "سبعمية",
    800: "تمنمية", 900: "تسعمية"
}

_SCALES = [
    (1_000_000_000, "مليار", "مليارين", "ملايارات"),
...
CURRENCY = {
    "LBP": ("ليرة", "ليرتين", "ليرات", "قرش", "قرشين", "قروش"),
    "ل.ل": ("ليرة", "ليرتين", "ليرات", "قرش", "قرشين", "قروش"),
    "ليرة": ("ليرة", "ليرتين", "ليرات", "قرش", "قرشين", "قروش"),
    "LL": ("ليرة", "ليرتين", "ليرات", "قرش", "قرشين", "قروش"),
    "£": ("باوند", "باوندين", "باوندات", "بنس", "بنسين", "بنسات"),
    "$": ("دولار", "دولارين", "دولارات", "سنت", "سنتين", "سنتات"),
    "USD": ("دولار", "دولارين", "دولارات", "سنت", "سنتين", "سنتات"),
    "€": ("يورو", "يوروين", "يوروهات", "سنت", "سنتين", "سنتات"),
    "EUR": ("يورو", "يوروين", "يوروهات", "سنت", "سنتين", "سنتات"),
}
...
# Levantine Month names
_AR_MONTHS = {
    1: "كانون التاني", 2: "شباط", 3: "آذار", 4: "نيسان", 5: "أيار", 6: "حزيران",
    7: "تموز", 8: "آب", 9: "أيلول", 10: "تشرين الأول", 11: "تشرين التاني", 12: "كانون الأول"
}
...
# ----------------------------
# Number to Lebanese Arabic
# ----------------------------

def int_to_lebanese_words(n: int) -> str:
    """Convert integer (>=0) into Lebanese Arabic words with better scaling."""
    if n < 0:
        return "ناقص " + int_to_lebanese_words(-n)
    if n < 20:
        return _AR_DIGITS[n]
    if n < 100:
        tens = (n // 10) * 10
    ...
    if n < 1000:
        ...
        return f"{_AR_HUNDREDS[hundreds]} و{int_to_lebanese_words(rest)}"
    ...
    for scale_value, scale_singular, scale_dual, scale_plural in _SCALES:
        if n >= scale_value:
            ...
            if major == 1:
                ...
                major_words = f"{int_to_lebanese_words(major)} {scale_plural}"
            else:
                major_words = f"{int_to_lebanese_words(major)} {scale_singular}"
            
            if rest:
                major_words += f" و{int_to_lebanese_words(rest)}"
            return major_words
...
def num_token_to_words(token: str) -> str:
    ...
    if "." in t:
        a, b = t.split(".", 1)
        a_words = int_to_lebanese_words(int(a)) if a else "صفر"
        ...
        out = int_to_lebanese_words(int(t))
...
def normalize_currency(text: str) -> str:
    ...
    Handles: "100 LBP", "100ل.ل", "$12.50", "€30"
    """
    def sym_first(m):
        sym = m.group("sym")
        num = m.group("num")
        ...
        if "." in t:
            ...
            if b_i == 0:
                return f"{int_to_lebanese_words(a_i)} {major_word}"
            
            minor_word = get_plural_form(b_i, minor_sg, minor_du, minor_pl)
            return f"{int_to_lebanese_words(a_i)} {major_word} و{int_to_lebanese_words(b_i)} {minor_word}"
    ...
    return re.sub(r"(?P<num>-?[\d٠-٩]+(?:[.,٫][\d٠-٩]+)?)\s*(?P<cur>LBP|USD|EUR|LL|ليرة|ل\.ل)", num_first, text)

def normalize_dates(text: str) -> str:
    """
    Handle date patterns:
    - 15/3/2024 -> "خمسطعش آذار ألفين وأربعة وعشرين"
    - 2024-03-15
    """
    def repl_slash(m):
        ...
        day_word = _AR_ORDINALS.get(day, int_to_lebanese_words(day))
        month_word = _AR_MONTHS.get(month, int_to_lebanese_words(month))
        
        if year:
            year_word = int_to_lebanese_words(year)
...
def normalize_time(text: str) -> str:
    """Enhanced time handling with more natural Lebanese expressions."""
    def repl(m):
        hh = int(normalize_arabic_digits(m.group("hh")))
        mm = int(normalize_arabic_digits(m.group("mm")))
        
        h_words = int_to_lebanese_words(hh)
        
        if mm == 0:
            return f"{h_words} عالقد"
        if mm == 15:
        ...
        if mm == 45:
            next_h = (hh + 1) % 24
            return f"{int_to_lebanese_words(next_h)} إلا ربع"
        
        # For other minutes
        m_words = int_to_lebanese_words(mm)
        if mm == 1:
...
def normalize_ranges(text: str) -> str:
    ...
    def repl(m):
        ...
        return f"من {num_token_to_words(a)} لـ {num_token_to_words(b)}"
...
def normalize_phone_like(text: str) -> str:
    ...
    Recognizes Lebanese phone patterns.
    """
    def repl(m):
    ...
    # Match general long digit sequences
...
def normalize_text_for_tts_lebanese(text: str) -> str:
    """
