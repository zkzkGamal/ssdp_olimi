...
# ----------------------------
# Core lexicon (Libyan Arabic)
# ----------------------------

_AR_DIGITS = {
    0: "صفر", 1: "واحد", 2: "اثنين", 3: "ثلاثة", 4: "أربعة", 5: "خمسة",
    6: "ستة", 7: "سبعة", 8: "ثمانية", 9: "تسعة", 10: "عشرة",
    11: "حداشر", 12: "اثناشر", 13: "ثلاثاشر", 14: "أربعتاشر", 15: "خمستاشر",
    16: "ستاشر", 17: "سبعتاشر", 18: "ثمانتاشر", 19: "تسعتاشر"
}
...
DECIMAL_WORD = "فاصلة"  # Libyan usually uses "Fasila" for decimals
PERCENT_WORD = "في المية"

# Currency words: Libyan Dinar (LYD) and Dirhams
CURRENCY = {
    "LYD": ("دينار", "دينارين", "دينار", "درهم", "درهمين", "دراهم"),
    "د.ل": ("دينار", "دينارين", "دينار", "درهم", "درهمين", "دراهم"),
    "دينار": ("دينار", "دينارين", "دينار", "درهم", "درهمين", "دراهم"),
...
# ----------------------------
# Number to Libyan Arabic
# ----------------------------

def int_to_libyan_words(n: int) -> str:
    """Convert integer (>=0) into Libyan Arabic words."""
    if n < 0:
        return "سالب " + int_to_libyan_words(-n)
    if n < 20:
        return _AR_DIGITS[n]
    if n < 100:
        tens = (n // 10) * 10
    ...
    if n < 1000:
        ...
        return f"{_AR_HUNDREDS[hundreds]} و{int_to_libyan_words(rest)}"
    ...
    for scale_value, scale_singular, scale_dual, scale_plural in _SCALES:
        if n >= scale_value:
            ...
            if major == 1:
                ...
                major_words = f"{int_to_libyan_words(major)} {scale_plural}"
            else:
                major_words = f"{int_to_libyan_words(major)} {scale_singular}"
            
            if rest:
                major_words += f" و{int_to_libyan_words(rest)}"
            return major_words
...
def normalize_currency(text: str) -> str:
    def sym_first(m):
        ...
        if "." in t:
            ...
            b_i = int((b + "000")[:3]) # Libya uses 1000 dirhams to 1 dinar
            
            major_word = get_plural_form(a_i, major_sg, major_du, major_pl)
            if b_i == 0:
                return f"{int_to_libyan_words(a_i)} {major_word}"
            
            minor_word = get_plural_form(b_i, minor_sg, minor_du, minor_pl)
            return f"{int_to_libyan_words(a_i)} {major_word} و{int_to_libyan_words(b_i)} {minor_word}"
    ...
    return re.sub(r"(?P<num>-?[\d٠-٩]+(?:[.,٫][\d٠-٩]+)?)\s*(?P<cur>LYD|USD|EUR|د\.ل|دينار)", num_first, text)
...
def normalize_dates(text: str) -> str:
    def repl_slash(m):
        ...
        day_word = _AR_ORDINALS.get(day, int_to_libyan_words(day))
        month_word = _AR_MONTHS.get(month, int_to_libyan_words(month))
        
        if year:
            year_word = int_to_libyan_words(year)
...
def normalize_time(text: str) -> str:
    def repl(m):
        ...
        h_words = int_to_libyan_words(hh)
        
        if mm == 0: return f"{h_words} تماماً"
        if mm == 15: return f"{h_words} وربع"
        if mm == 30: return f"{h_words} ونص"
        if mm == 45:
            next_h = (hh + 1) % 24
            return f"{int_to_libyan_words(next_h)} إلا ربع"
        
        m_words = int_to_libyan_words(mm)
...
def normalize_text_for_tts_libyan(text: str) -> str:
